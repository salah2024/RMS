function ShowPayKaniEzafeBaha() {
    ...
    $.ajax({
        type: "POST",
        url: "/PayKaniInfoForBarAvords/GetEzafeBaha",
        data: JSON.stringify(vardata),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            lstNoePayKaniEzafeBaha = response.lstNoePayKaniEzafeBaha;
            lstPKForBEB = response.lstPKForBEB;


            $container = $('#ViewPayKaniEzafeBaha');

            var preselectedIds = [];

            $.each(lstNoePayKaniEzafeBaha, function () {
                debugger;
                ...
                const isInLstPKForBEB = lstPKForBEB.some(x => x.noeKhakBardariEzafeBahaId === item.id);
                if (preselectedIds.includes(item.id) || isInLstPKForBEB) {
                    $checkbox.prop("checked", true);
                }

                // متن لیبل
                const $text = $("<span/>").text(item.noePayKaniEzafeBaha);
                ...
                $checkbox.on("change", function () {
                    debugger;
                    if ($(this).is(":checked")) {
                        SaveKhEzafeBaha(item.id);
                    } else {
                        DeleteKhEzafeBaha(item.id);
                    }
                });
...
function ShowEzafeBahaRizMetre(EzafeBahaId) {
    debugger;
    BarAvordUserId = $('#HDFBarAvordUserID').val();
    Year = $('#HDFYear').val();
    ...
    $.ajax({
        type: "POST",
        url: "/PayKaniInfoForBarAvords/GetPK_EzafeBaha",
        data: JSON.stringify(vardata),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
...
function SaveKhEzafeBaha(EzafeBahaId) {
    ...
    $.ajax({
        type: "POST",
        url: "/PayKaniInfoForBarAvords/SaveEzafeBahaPayKani",
        data: JSON.stringify(vardata),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            if (response == "OK") {
                ShowEzafeBahaRizMetre(EzafeBahaId);
            }
        },
        error: function (response) {
            toastr.error('مشکل در درج اضافه بها', 'خطا');
